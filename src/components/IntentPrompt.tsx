...
const IntentPrompt = ({ expiringTotal = 0, earliestExpiry, expiryDismissed, onDismissExpiry }: IntentPromptProps) => {
  ...
  return (
    <section className="relative min-h-[70vh] flex flex-col justify-center overflow-hidden">
      {/* Background video */}
      <video
        src="https://gentle-horse-81938c.netlify.app/Paint.mp4"
        autoPlay
        loop
        muted
        playsInline
          ...
          <div className="hidden md:flex absolute left-4 lg:left-8 top-1/2 -translate-y-1/2 z-10 animate-slide-in-left">
            <div className="w-[176px] bg-card/60 backdrop-blur-md border border-border/40 rounded-2xl p-4 space-y-3">
              ...
              {!expiryDismissed && expiringTotal > 0 && (
                <div className="flex items-center gap-1.5 text-[11px] text-amber-600 dark:text-amber-400 animate-fade-in">
                  <span>⏳</span>
                  <span><span className="font-semibold">{expiringTotal} Ink</span> expires in 60 days</span>
            ...
            <div className="md:hidden mb-4 animate-fade-in flex flex-wrap justify-center gap-1.5">
              ...
              {!expiryDismissed && expiringTotal > 0 && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-300/30 text-xs animate-fade-in">
                  <span className="text-amber-600 dark:text-amber-400">⏳ <span className="font-semibold">{expiringTotal} Ink</span> expires in 60 days</span>
                </span>
              )}
            </div>
          )}
